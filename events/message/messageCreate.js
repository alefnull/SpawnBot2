...
module.exports = {
	...
	async run(message, client) {
		...
		let command = client.commands.get(cmd_name) || client.commands.find(cmd => cmd.aliases && cmd.aliases.includes(cmd_name));
		...
		if (custom_command_response) {
			const embed = new MessageEmbed()
				.setColor('#0000ee')
				.setDescription(custom_command_response);
			log_info(`${blue('[' + message.guild.name + ']')} ${magenta(message.author.tag)} ran the ${blue('custom command')}: ${yellow(cmd_name)}.`);
		...
		if (command.permissions) {
			...
			if (!author_permissions || !author_permissions.has(command.permissions)) {
				const embed = new MessageEmbed()
					...
					.setDescription(`You need the following permissions to use this command: \`${command.permissions}\``);
		...
		try {
			log_info(`${blue('[' + message.guild.name + ']')} ${magenta(message.author.tag)} ran the ${blue('command')}: ${yellow(command.name)}.`);
