...
module.exports = {
  ...
  run(member) {
    ...
    log_info(
      `${blue('[' + member.guild.name + ']')} ${magenta(member.user.tag)} has left the server.`
    );
    ...
    const embed_log = new MessageEmbed()
      ...
      .setDescription(
        `${member.user.username}#${member.user.discriminator} left ${member.guild.name}`
      );

    member.guild.channels.cache
      .find((chan) => chan.name === server.logging_channel)
      .send({ embeds: [embed_log] });
  }
};
