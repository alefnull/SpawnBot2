...
const dfmt = function(dt, fmt, utc) {
	...
	var m = utc ? dt.getUTCMinutes() : dt.getMinutes();
	fmt = fmt.replace(/(^|[^\\])mm+/g, "$1" + ii(m));
	fmt = fmt.replace(/(^|[^\\])m/g, "$1" + m);

	var s = utc ? dt.getUTCSeconds() : dt.getSeconds();
	fmt = fmt.replace(/(^|[^\\])ss+/g, "$1" + ii(s));
	fmt = fmt.replace(/(^|[^\\])s/g, "$1" + s);

	var f = utc ? dt.getUTCMilliseconds() : dt.getMilliseconds();
	...
	var day = (utc ? dt.getUTCDay() : dt.getDay()) + 1;
