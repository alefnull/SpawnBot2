...
module.exports = {
  ...
  run(client, msg, args) {
    ...
    const embed = new MessageEmbed().setColor('#0000ee').setTitle('Custom Commands');

    // check if the user has permission to use this command
    if (!msg.member.permissions.has(this.permission)) {
      embed.setDescription('You do not have permission to use this command.').setColor('#ee0000');
    ...
    if (!args.length) {
      ...
      if (client.custom_commands.size === 0) {
        embed.setDescription('There are no custom commands in the collection.').setColor('#ee0000');
        return msg.channel.send({ embeds: [embed] });
      }
      ...
      embed.setDescription(`\`${list.join(', ')}\``).setColor('#0000ee');
      return msg.channel.send({ embeds: [embed] });
    }
    ...
    switch (subcmd) {
      case 'add':
        ...
        if (!custcmd || !resp) {
          embed.setDescription('Please specify a command and response to add.').setColor('#ee0000');
          return msg.channel.send({ embeds: [embed] });
        }
        ...
        if (client.commands.has(custcmd) || client.custom_commands.has(custcmd)) {
          embed
            .setDescription(`The command \`${prefix}${custcmd}\` already exists.`)
            .setColor('#ee0000');
          return msg.channel.send({ embeds: [embed] });
        }
        ...
        embed
          .setDescription(`The command \`${custcmd}\` has been added to the custom commands list.`)
          .setColor('#00ee00');
        return msg.channel.send({ embeds: [embed] });
      ...
      case 'del':
        ...
        if (!custcmd) {
          embed.setDescription('Please specify a command to remove.').setColor('#ee0000');
          return msg.channel.send({ embeds: [embed] });
        }
        ...
        if (!client.custom_commands.has(custcmd)) {
          embed
            .setDescription(`The command \`${custcmd}\` is not in the custom commands list.`)
            .setColor('#ee0000');
          return msg.channel.send({ embeds: [embed] });
        }
        ...
        embed
          .setDescription(
            `The command \`${custcmd}\` has been removed from the custom commands list.`
          )
          .setColor('#00ee00');
        return msg.channel.send({ embeds: [embed] });
      ...
      default: {
        // check if the custom commands collection is empty
        if (client.custom_commands.size === 0) {
          embed.setDescription('There are no custom commands in the list.').setColor('#ee0000');
          return msg.channel.send({ embeds: [embed] });
        }
