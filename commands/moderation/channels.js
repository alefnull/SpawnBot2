...
module.exports = {
  ...
  run(client, msg, args) {
    ...
    const embed = new MessageEmbed().setColor('#0000ee').setTitle('Channel Access');
    ...
    if (!msg.member.permissions.has(this.permission)) {
      ...
      log_warn(
        `${blue('[' + msg.guild.name + ']')} ${magenta(msg.author.tag)} tried to use the ${yellow(
          this.name
    ...
    if (!args.length) {
      if (server.channel_restrictions) {
        embed.setDescription('Channel restrictions are enabled.').setColor('#ee0000');
      } else {
        embed.setDescription('Channel restrictions are disabled.').setColor('#00ee00');
      }
      return msg.channel.send({ embeds: [embed] });
    }
    ...
    switch (subcmd) {
      ...
      case 'remove':
        ...
        server.allowed_channels[msg.guild.name].splice(
          server.allowed_channels[msg.guild.name].indexOf(channel),
          1
        );
      ...
      case 'list': {
        ...
        embed.setDescription(
          `The only ***${msg.guild.name}*** channels I'm currently allowed to speak in are:\n\`${channel_list}\``
        );
        return msg.channel.send({ embeds: [embed] });
      }

      default:
        embed.setDescription('Please specify a valid sub-command.').setColor('#ee0000');
        return msg.channel.send({ embeds: [embed] });
    }
  }
};
