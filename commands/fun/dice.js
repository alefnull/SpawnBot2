  ...
  run(client, msg, args) {
    ...
    if (cmd === 'coin' || cmd === 'flip') {
      ...
      if (isNaN(flips) || flips < 1 || flips > 100) {
        embed.setColor('#ee0000').setDescription('Invalid number of flips.');
        return msg.channel.send({ embeds: [embed] });
      }
      ...
      embed
        .setColor('#0000ee')
        .setDescription(
          `${msg.author.username} flipped ${flips} coin${
            flips > 1 ? 's' : ''
          } and got:\n\n**${results.join(', ')}**`
        );
      return msg.channel.send({ embeds: [embed] });
    ...
    if (!args[0]) {
      embed
        ...
        .setColor('#ee0000')
        .setDescription(
          'You must specify which dice to roll. (e.g. \'2d6\' for two six-sided dice, or \'10\' for a single ten-sided die.)'
        );
      return msg.channel.send({ embeds: [embed] });
    }
    ...
    if (args[0].match(/^(\d+)d(\d+)$/)) {
      ...
      if (isNaN(num) || num < 1 || num > 100) {
        embed.setTitle('Dice Roller').setColor('#ee0000').setDescription('Invalid number of dice.');
        return msg.channel.send({ embeds: [embed] });
      }
      ...
      embed
        ...
        .setColor('#0000ee')
        .setDescription(
          `${msg.author.username} rolled ${num} ${sides}-sided ${
            num > 1 ? 'dice' : 'die'
          } and got:\n\n**${results.join(', ')}**`
        );
      return msg.channel.send({ embeds: [embed] });
    ...
    if (args[0].match(/^(\d+)$/)) {
      ...
      embed
        ...
        .setColor('#0000ee')
        .setDescription(
          `${msg.author.username} rolled 1 ${sides}-sided die and got:\n\n**${
