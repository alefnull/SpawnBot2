...
module.exports = (client) => {
  ...
  for (let folder of command_folders) {
    const command_files = readdirSync(`./commands/${folder}`).filter((file) =>
      file.endsWith('.js')
    );
  ...
  if (existsSync('./data/custom.json')) {
    ...
    for (let command in custom) {
      if (Object.prototype.hasOwnProperty.call(custom, command)) {
  ...
  log_info(
    `Registered ${blue(cust_cmd_list.length)} custom command(s): ${yellow(
      cust_cmd_list.join(', ')
    )}`
  );
};
